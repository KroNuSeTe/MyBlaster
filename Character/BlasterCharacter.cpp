...
void ABlasterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(AimAction, ETriggerEvent::Started, this, &ABlasterCharacter::AimKeyPressed);
		EnhancedInputComponent->BindAction(AimAction, ETriggerEvent::Completed, this, &ABlasterCharacter::AimKeyReleased);
		EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Started, this, &ABlasterCharacter::FireKeyPressed);
		EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Completed, this, &ABlasterCharacter::FireKeyReleased);
		EnhancedInputComponent->BindAction(ReloadAction, ETriggerEvent::Started, this, &ABlasterCharacter::ReloadKeyPressed);
		EnhancedInputComponent->BindAction(ThrowGrenadeAction, ETriggerEvent::Completed, this, &ABlasterCharacter::ThrowGrenadeKeyPressed);
	}
...
void ABlasterCharacter::MulticastElim_Implementation(bool bPlayerLeftGame)
{
	...
	if (BlasterPlayerController)
	{
		...
		//Spawn The Message You Were Eliminated!
		BlasterPlayerController->SetHUDEliminationMessage();
		if (CombatComp && CombatComp->EquippedWeapon)
		{
			BlasterPlayerController->SetHUDRifleOutIconAnim();
		}		
	}
	// Disable collision
...
void ABlasterCharacter::EquipKeyPressed()
{
	...
	if (CombatComp)
	{
		...
		bool bSwap = CombatComp->ShouldSwapWeapons() && 
			!HasAuthority() && 
...
void ABlasterCharacter::FireKeyPressed()
{
	if (CombatComp && CombatComp->bHoldingTheFlag) return;
	if (bDisableGameplay) return;
...
void ABlasterCharacter::FireKeyReleased()
{
	if (CombatComp && CombatComp->bHoldingTheFlag) return;
	if (bDisableGameplay) return;
...
void ABlasterCharacter::ReloadKeyPressed()
{
	if (CombatComp && CombatComp->bHoldingTheFlag) return;
	if (bDisableGameplay) return;
	if (CombatComp)
...
void ABlasterCharacter::ThrowGrenadeKeyPressed()
{
	if (CombatComp)
	{
...
void ABlasterCharacter::DissolveEffect(bool IsReverse)
{
	if (IsReverse) {
		ReverseDissolve();
...
void ABlasterCharacter::ReverseDissolve()
{
	DissolveTrack.BindDynamic(this, &ABlasterCharacter::UpdateDissolveMaterial);
	if (ReverseDissolveCurve && DissolveTimeline)
	{
		DissolveTimeline->AddInterpFloat(ReverseDissolveCurve, DissolveTrack);
		DissolveTimeline->Play();
	}
}
...
void ABlasterCharacter::SetTeamColor(ETeam Team)
{
	...
	case ETeam::ET_NoTeam:
		...
		DissolveMaterialInstance = BlueDissolverMatInst;
		break;
...
void ABlasterCharacter::MulticastGainedTheLead_Implementation()
{
	GetWorldTimerManager().SetTimer(
		CrownSpawnTimer,
		this,
		&ABlasterCharacter::CrownTimerFinished,
		CrownTimerDelay
	);
}

void ABlasterCharacter::CrownTimerFinished()
{
	if (CrownSystem == nullptr) return;
	const USkeletalMeshSocket* CrownSocket = GetMesh()->GetSocketByName(FName("CrownSocket"));
	if (CrownSocket == nullptr) return;
	if (CrownComponent == nullptr)
	{
		CrownComponent = UNiagaraFunctionLibrary::SpawnSystemAttached(
			...
			GetMesh(),
			FName(),
			CrownSocket->GetSocketLocation(GetMesh()),
			FRotator(),
